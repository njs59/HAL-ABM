...
public class ExampleModel extends AgentGrid2D<InitialModel.ExampleCell> {
    ...
    public double ACID_SECRETION_RATE = 0.02;
    public double ACID_UPTAKE = -0.0001;
   //public double ACID_DECAY = -0.1 * TIMESTEP;
    public double ACID_DECAY = -0.1 * TIMESTEP;
    public double ACID_EFFECT = 0.02;
    //public double ACID_EFFECT = (77.0/1350)-(797.0/14850);
    public double ACID_EFFECT_DIV_PROP = 0.5;
    //Above is the effect that acid has on the chance of division (1.0 is no effect 0.0 is completely stops division)
    public double ACID_RESISTANCE_EFFECT_DEATH = 0.5;
    public double ACID_RESISTANCE_EFFECT_DIV = 0.5;
...
//        }
        ...
        if (adaptive == 1){
            if (Pop() > 5*initial_no_cells && (tick - DRUG_START) > 0){
                //drug diffuses
...
class ExampleCell extends AgentSQ2Dunstackable<ExampleModel> {
    ...
    public void CellStep() {
        ...
        if (this.type == RESISTANT) {
            ...
            divProb = G.DIV_PROB_RES - G.getAcid().Get(Isq()) * G.ACID_EFFECT * G.ACID_EFFECT_DIV_PROP;
            ...
            divProb = G.DIV_PROB_SEN - G.getAcid().Get(Isq()) * G.ACID_EFFECT * G.ACID_EFFECT_DIV_PROP * G.ACID_RESISTANCE_EFFECT_DIV;
        }
